...
export const getUserLeagues = functions.https.onCall(async (data) => {
  const { userId, sport, season } = data;
  return await makeRequest(`/user/${userId}/leagues/${sport}/${season}`);
});

// League endpoints
export const getLeague = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}`);
});

export const getLeagueRosters = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/rosters`);
});

export const getLeagueUsers = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/users`);
});

export const getLeagueMatchups = functions.https.onCall(async (data) => {
  const { leagueId, week } = data;
  return await makeRequest(`/league/${leagueId}/matchups/${week}`);
});

export const getLeagueWinnersBracket = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/winners_bracket`);
});

export const getLeagueLosersBracket = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/losers_bracket`);
});

export const getLeagueTransactions = functions.https.onCall(async (data) => {
  const { leagueId, round } = data;
  return await makeRequest(`/league/${leagueId}/transactions/${round}`);
});

export const getLeagueTradedPicks = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/traded_picks`);
});

// Draft endpoints
export const getUserDrafts = functions.https.onCall(async (data) => {
  const { userId, sport, season } = data;
  return await makeRequest(`/user/${userId}/drafts/${sport}/${season}`);
});

export const getLeagueDrafts = functions.https.onCall(async (data) => {
  const { leagueId } = data;
  return await makeRequest(`/league/${leagueId}/drafts`);
});

export const getDraft = functions.https.onCall(async (data) => {
  const { draftId } = data;
  return await makeRequest(`/draft/${draftId}`);
});

export const getDraftPicks = functions.https.onCall(async (data) => {
  const { draftId } = data;
  return await makeRequest(`/draft/${draftId}/picks`);
});

export const getDraftTradedPicks = functions.https.onCall(async (data) => {
  const { draftId } = data;
  return await makeRequest(`/draft/${draftId}/traded_picks`);
});

// NFL State
export const getNflState = functions.https.onCall(async () => {
  return await makeRequest('/state/nfl');
});

// Players
export const getAllPlayers = functions.https.onCall(async () => {
  return await makeRequest('/players/nfl');
});

export const getTrendingPlayers = functions.https.onCall(async (data) => {
